...
export default function CheckIn() {
   ...
   const selectedDow = watchedDate
      ? parseISO(watchedDate).getDay()
      : new Date().getDay();
   const scheduledMeds = medications
      .filter(
         (m) =>
            m.isActive && m.schedules.some((s) => s.dayOfWeek === selectedDow),
      )
      .map((m) => ({
         id: m.id,
         name: m.name,
            ...
            for (const [symptom, score] of scores) {
               if (score >= 7) {
                  ...
                  if (relevant.length > 0)
                     highSymptoms.push({ symptom, meds: relevant });
               }
            }
   ...
   const tourSteps = [
      {
         ...
         content:
            "Welcome to Willow! Each day, log how you're feeling on this page. Your entries help your care team understand your symptoms over time.",
         disableBeacon: true,
      },
      {
         target: "#checkin-sliders",
         content:
            "Rate your mood, pain, fatigue, and nausea on a scale of 1–10 by dragging each slider. Higher numbers mean more intense symptoms.",
         disableBeacon: true,
      },
      {
         target: "#checkin-medications",
         content:
            "If you have medications scheduled for today, check them off here. Your adherence is tracked over time and shown on the Medications page.",
         disableBeacon: true,
      },
      {
         target: "#checkin-weight",
         content:
            "Optionally record your weight in kg. This is tracked separately with its own trend graph.",
         disableBeacon: true,
      },
      {
         target: "#notes",
         content:
            "Add any extra notes — side effects, how you slept, anything on your mind. This is just for you.",
         disableBeacon: true,
      },
      {
         target: "#checkin-submit",
         content:
            "When you're ready, tap Save Entry. You can only submit one entry per date, but you can always edit it later from History.",
         disableBeacon: true,
      },
   ];
                     ...
                     <div id="checkin-medications" className="px-6 py-5">
                        ...
                        <div className="flex flex-col gap-2.5">
                           {scheduledMeds.map((med) => {
                              ...
                              const isPending =
                                 markTaken.isPending || unmarkTaken.isPending;
                              return (
                                 <button
                                    key={med.id}
                                    type="button"
         ...
         >
            <DialogContent>
               <DialogHeader>
                  ...
                  <DialogDescription>
                     You logged a high score for the following symptoms. Your
                     prescribed medications may help — speak to your care team
                     if you're unsure.
                  </DialogDescription>
               </DialogHeader>
