      ...
      <>
         ...
         <div className="max-w-2xl mx-auto w-full px-4 sm:px-6 py-8 flex flex-col gap-3">
            ...
            {pageItems.map((checkIn) => {
               const { weekday, date } = formatDate(checkIn.date);
               return (
                  <div key={checkIn.id} className="rounded-2xl border bg-card overflow-hidden">
                     <div className="flex items-center justify-between px-5 py-3.5 border-b">
                        <div>
                           <p className="text-xs font-semibold tracking-widest uppercase text-muted-foreground leading-none mb-0.5">
                              {weekday}
                           </p>
                     ...
                     <div className="grid grid-cols-4 px-5 py-4 gap-2">
                        {metrics.map((key) => (
                           <div key={key} className="text-center">
                              ...
                              <div className="h-1 rounded-full bg-border overflow-hidden mx-auto max-w-[40px]">
                                 <div
                                    className="h-full rounded-full bg-primary/50 transition-all"
                                    style={{ width: `${(checkIn[key] / 10) * 100}%` }}
                                 />
                              </div>
                           </div>
                        ))}
